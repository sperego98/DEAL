"""
OTF training engine for DEAL, code adapted from https://github.com/mir-group/flare/blob/master/flare/learners/otf.py
"""
...
class OTF_DEAL:
    ...
    Args:
        ...
        wall_time (int): time in seconds after which terminate the program
            and save the files for restart.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.md = FakeMD(
            atoms=self.atoms, timestep=1, trajectory=trajectory, **md_kwargs
        )
        ...
        # set md
        self.dt = 0.001
        self.number_of_steps = np.sum(self.md.stat_num)
        ...
        self.dft_xyz = self.output_name + "_selected.xyz"
        self.checkpt_files = [
            self.checkpt_name,
            self.flare_name,
            self.dft_name,
    ...
    def run(self):
        ...
        try: 
            import tqdm
            pbar = tqdm.tqdm(range(self.number_of_steps))
        except ImportError:
            pbar = range(self.number_of_steps)
        for step in pbar:
            if hasattr(pbar, "set_postfix_str"):
                pbar.set_postfix_str(f"Selected: {len(self.dft_frames)}")
            else:
                print(f"Frame: {step}/{self.number_of_steps}, Selected: {len(self.dft_frames)}")
        ...
        #while self.curr_step < self.number_of_steps:
            ...
            # check if time > wall_time 
            if self.wall_time is not None:
                if time.time() - self.start_time > self.wall_time:
                    print(f'[WARNING]: Wall-time limit ({self.wall_time}) reached.')
                    f = logging.getLogger(self.output.basename + "log")
                    f.info(f'[WARNING]: Wall-time limit ({self.wall_time}) reached.')
                    break
        ...
        # create chemiscope input
        try:
            import chemiscope
            from utils import create_chemiscope_input
            chemiscope_input = create_chemiscope_input(self.dft_xyz, self.output.basename+"_chemiscope.json.gz")
        
            f = logging.getLogger(self.output.basename + "log")
            f.info(f'Created chemiscope input: {chemiscope_input}')
        except ImportError as e:
            print(e)
            f.info(f'Chemiscope is not available, no file prepared.')
            pass
    ...
    def train_gp(self):
        ...
        self.gp.train(logger_name=None) # self.output.basename + "hyps")
    ...
    def record_state(self):
        ...
        if self.md_engine == "Fake" and not self.dft_step and self.write_model >= 3:
            tic = time.time()
            e_mae, e_mav, f_mae, f_mav, s_mae, s_mav = compute_mae(
                self.atoms,
                self.output.basename,
    ...
    def record_dft_data(self, structure, target_atoms):
        ...
        structure.info["step"] = self.curr_step
